...
body(
    ...
    '저소득과 독거가 중첩되는 이중 취약 구조가 심각함을 알 수 있다. '
...
body(
    ...
    '바 없다. 집중(concentration)과 분리(segregation)는 다른 현상으로, 저소득 독거노인이 '
    '특정 동에 많다는 것이 곧 다른 집단과 구조적으로 분리되어 있음을 의미하지 않는다. '
    '둘째, 거주지 분리 측도를 적용한 연구들은 대부분 인종·민족 분리 맥락에 '
...
body(
    ...
    '낮은 분리, 0.30~0.60: 중간, > 0.60: 높은 분리)으로는 낮은 분리 수준에 해당하지만, '
    '독거노인 내부 소득 분리(D = 0.0859, 4.3절)보다 높은 값으로, 독거 여부 자체보다 소득·고령·1인 가구 '
