...
p_iso <- ggplot(data_sf) +
  ...
  theme_choropleth(
    ...
    "저소득 독거노인 vs 전체 주민 | 전역 고립지수 = 0.0174 | bandwidth = 1,500 m"
  )
...
p_diss <- ggplot(data_sf) +
  ...
  theme_choropleth(
    ...
    "저소득 vs 비교집단 독거노인 | 전역 상이지수 = 0.0866 | bandwidth = 1,500 m"
  )
...
for (i in seq_along(bw_list)) {
  cat(sprintf("  %d m...\n", bw_list[i]))
  r_loc        <- RSG_D_Local(data = data_sf, group_a = "저소득", group_b = "비교집단",
                               total = "독거_전체", bandwidth = bw_list[i], dist_matrix = d_mat)
  r_glob       <- RSG_D_Global(data = data_sf, group_a = "저소득", group_b = "비교집단",
                                total = "독거_전체", bandwidth = bw_list[i], dist_matrix = d_mat)
