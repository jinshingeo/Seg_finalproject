...
# ── 1. 시설 데이터 로드 ────────────────────────────────────────────────────────
path_fac <- "../독거노인관련_0527/노인여가+복지시설(동별)_20260527210745.xlsx"
...
seg_df <- data_sf %>%
  ...
  mutate(Local_Isolation     = results$I_local$index,
         Local_Dissimilarity = results$D_local$index) %>%
  select(구, 동, 저소득, 비교집단, 독거_전체, 총인구,
...
p_diss_fac <- ggplot(joined %>% filter(!is.na(diss_quad)),
  ...
  scale_y_continuous(limits = c(0, y_max)) +
  labs(title    = "2단계: 소득 기반 분리 × 노인여가복지시설 밀도",
       subtitle = "동 단위 실측 353개 동 | ◆=구 단위 평균 (중랑·노원·서초·강동구, 동 단위 데이터 없음)",
