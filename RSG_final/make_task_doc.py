...
bullet('참고문헌은 처음부터 EndNote로 삽입 — 나중에 수동 정리하지 않아도 되도록')
...
bullet('EndNote 라이브러리로 전체 참고문헌 등록 후 지도학회지 스타일로 출력')
bullet('참고문헌 파일(Word) 별도 전달 — 서기에게 목요일까지')
...
sub_bullet('참고문헌 EndNote 출력본과 본문 인용 번호 최종 대조')
...
# 3-5 EndNote
para('⑤ 참고문헌 EndNote 사용 필수', size=10.5, bold=True,
     color=RGBColor(0x1F, 0x3D, 0x6E), space_before=10)
bullet('모든 인용은 처음부터 EndNote로 삽입 — 수동 입력 금지')
bullet('스타일: 지도학회지 인용 스타일 적용 (승원이 라이브러리 공유 예정)')
bullet('나중에 수동 정리하는 것이 훨씬 번거로우니 처음부터 필수')
